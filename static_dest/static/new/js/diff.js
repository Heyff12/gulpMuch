...
$('.js_history').get(0) && (~ function() {
    ...
    $(document).ready(function() {
        ...
        //get_history();
        // 初始化插件内容
        var showDom = document.querySelector('#f_companydate');
        showDom.dataset['year'] = nowYear;
        showDom.dataset['month'] = nowMonth;
        showDom.value = nowYear + '-' + nowMonth;
    });
    //关闭提现失败错误弹框
    $('#alert_error').on('click', function() {
        $(this).hide();
        $('.zheceng').hide();
        $('.zheceng1').hide();
    });
    ...
    //点击时间
    $('#f_companydate').on('click', function() {
        var showDom = document.querySelector('#f_companydate');
        var year = showDom.dataset['year'];
        var month = showDom.dataset['month'];

        var bankSelect = new IosSelect(2, [yearData, monthData], {
            ...
            twoLevelId: month,
            //showLoading: true,
            callback: function(year_data, month_data) {
                showDom.dataset['year'] = year_data.id;
                showDom.dataset['month'] = month_data.id;
                showDom.value = year_data.id + '-' + month_data.id;
                $('#birthday').val(year_data.id + '-' + month_data.id);
            }
        });
    });
    //根据日期查询
